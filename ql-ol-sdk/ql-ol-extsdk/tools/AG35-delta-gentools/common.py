...
def LoadInfoDict(zip, type):
  ...
  try:
    for line in zip.read("META/misc_info.txt").split("\n"):
      ...
      k, v = line.split("=", 1)
      d[k] = v
    print "[becking][common.py]LoadInfoDict d[k]", d
  except KeyError:
  ...
  if "tool_extensions" not in d:
    try:
      ...
      print "[becking][common.py]LoadInfoDict d[tool_extensions]", d["tool_extensions"]    #This code is not executed
    except KeyError:
  ...
  try:
    ...
    for line in data.split("\n"):
      ...
      if name == "blocksize":
        ...
        d[name + "_size"] = value
    print "[becking][common.py]LoadInfoDict d[name = size]", d    #This code is not executed
  except KeyError:
    pass
  ...
  print "[becking][common.py]makeint d[fstab]", d["fstab"]
  return d
...
def LoadRecoveryFSTab(zip, type):
  ...
  for line in data.split("\n"):
    ...
    p.length = 0
    options = None
    print "[becking][common.py]LoadRecoveryFSTab p.mount_point ", p.mount_point
    if len(pieces) >= 4:
    ...
    d[p.mount_point] = p
    print "[becking][common.py]LoadRecoveryFSTab p.mount_point ", d[p.mount_point]
  return d
...
"""
2016/07/09 [becking] add for get system/firmware/image/modem.b21
"""
def GetModemImage(name, prebuilt_name, unpack_dir, tree_subdir):
  """Return a File object (with name 'name') with the desired bootable
  image.  Look for it in 'unpack_dir'/BOOTABLE_IMAGES under the name
  'prebuilt_name', otherwise construct it from the source files in
  'unpack_dir'/'tree_subdir'."""

  prebuilt_path = os.path.join(unpack_dir, "SYSTEM", prebuilt_name)
  if os.path.exists(prebuilt_path):
    print "using prebuilt %s..." % (prebuilt_name,)
    return File.FromLocalFile(name, prebuilt_path)
  else:
...
def UnzipTemp(filename, pattern=None):
  ...
  if m:
    ...
    filename = m.group(1)
    print "[becking]unzip_to_dir filename", filename   #This code is not executed
  else:
