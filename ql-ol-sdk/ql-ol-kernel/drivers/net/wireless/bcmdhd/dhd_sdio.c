...
dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	DHD_ERROR(("%s: timeleft=%d  bus->rxlen=%d\n", __FUNCTION__,timeleft,bus->rxlen));
...
dhdsdio_isr(void *arg)
{
	...
	bus->intrcount++;
	DHD_INFO(("%s: bus->intrcount++ =%d\n", __FUNCTION__, bus->intrcount));
	bus->ipend = TRUE;
	...
	if (bus->intr) {
		DHD_INFO(("%s: disable SDIO interrupts , bus->intr=%d\n", __FUNCTION__, bus->intr));
	} else {
...
dhdsdio_dpc(dhd_bus_t *bus)
{
	...
	DHD_ERROR(("%s: bus->sleeping: %d\n", __FUNCTION__, (bus)->sleeping));
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	printk("%s: venid 0x%04x devid 0x%04x bus_no %d  slot %d  func %d  bustype %d \n", __FUNCTION__, venid, devid,bus_no,slot,func,bustype);
	...
	if (bus->intr) {
		...
		DHD_INFO(("%s: disable SDIO interrupts (not interested yet)\n", __FUNCTION__));
		bcmsdh_intr_disable(sdh);
		...
		DHD_INFO(("%s: registered SDIO interrupt function ok\n", __FUNCTION__));
	} else {
		DHD_ERROR(("%s: SDIO interrupt function is NOT registered due to polling mode\n",
		           __FUNCTION__));
	}
