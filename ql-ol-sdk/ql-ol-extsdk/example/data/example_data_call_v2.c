...
int main(int argc, char **argv)
{
	...
	while(-1 != (opt = getopt(argc, argv, "i:f:r"))) {
		switch(opt) {
			case 'i':
				if(atoi(optarg) <0 || atoi(optarg) > 5) {
					printf("invalid Profile ID\n");
					return -1;
				}
				g_call.profile_idx = atoi(optarg);
				break;
			case 'f':
				if(!strcmp(optarg, "v4")) {
