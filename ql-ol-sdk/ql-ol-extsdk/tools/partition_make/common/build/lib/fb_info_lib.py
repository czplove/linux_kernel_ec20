...
class fbInfo:
   ...
   #  Constructor
   #==============================================================================
   def __init__(self, on_linux,logger,fb_nearest = False):
      self.fbPath = None
   ...
   def __set_fbPath(self, on_linux, buildID):
      try:
         ...
         if on_linux:
            self.fb_log("fb_info_lib: Trying /prj/qct/asw/qctss/linux/bin/FindBuild") #Debug Statement - self.fb_log FindBuild Command
            self.fbPath = "/prj/qct/asw/qctss/linux/bin/FindBuild"
         else:
            self.fb_log("fb_info_lib: Trying \\\\stone\\aswcrm\\smart\\nt\\bin\\FindBuild") #Debug Statement - self.fb_log FindBuild Command
