...
void msm_setup_devices_work(struct power_supply *psy, int online) {
	struct msm_otg *motg = container_of(psy, struct msm_otg, usb_psy);
		...
		unsigned gpio = 0;
		if (gpio) {
			if(!gpio_request(gpio, "usb_state")) {
				gpio_direction_input(gpio);
				dev_info(&motg->pdev->dev, "gpio_direction_input(%d) = %d\n", gpio, (!!gpio_get_value(gpio)));
				motg->pdata->default_mode = motg->pdata->mode = (!!gpio_get_value(gpio)) ? USB_PERIPHERAL : USB_HOST;
				gpio_free(gpio);
			}
		} else {
		dev_info(&motg->pdev->dev, "vbus_state = %d\n", online);
		motg->pdata->default_mode = motg->pdata->mode = online ? USB_PERIPHERAL : USB_HOST;
		}
#endif	

		msm_otg_setup_devices(motg->pdev, motg->pdata->mode, true);
}
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
#if 1
		//delay call msm_otg_setup_devices() until know vbus state
#else
