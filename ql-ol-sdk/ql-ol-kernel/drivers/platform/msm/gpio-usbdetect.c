...
void msm_setup_devices_work(struct power_supply *psy, int online);
#endif
...
#ifdef QUECTEL_SLEEP_CTRL
	//mdelay(200); //do not mdelay/msleep here, or can not detect emmc	
#endif
...
#ifdef QUECTEL_SLEEP_CTRL
	msm_setup_devices_work(usb->usb_psy, !!irq_read_line(usb->vbus_det_irq));
	INIT_DELAYED_WORK(&usb->vbus_det_work, vbus_det_work_func_t);
	//wait usb phy init ok, then tell usb phy the vbus state.
	//or if boot up without usb cable, EC20 do not call msm_otg_runtime_suspend() and release msm_otg wakelock
	schedule_delayed_work(&usb->vbus_det_work, msecs_to_jiffies(200)); //if want work device mode, but do not connect vbus, comment this line
#else
	/* Read and report initial VBUS state */
	local_irq_save(flags);
	gpio_usbdetect_vbus_irq(usb->vbus_det_irq, usb);
