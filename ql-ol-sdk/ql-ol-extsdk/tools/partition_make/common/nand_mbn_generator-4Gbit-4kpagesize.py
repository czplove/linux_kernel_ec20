...
#  $Header: //components/rel/boot.bf/3.1.2/boot_images/core/storage/tools/nandbootmbn/nand_mbn_generator.py#3 $
#  $DateTime: 2016/12/14 01:47:46 $ 
#  $Author: pwbldsvc $
...
def ParseToCreateOutputFiles(filename, block_size_in_kb, page_size_in_kb, total_page_count):
 ...
 #quectel marked   elem = Element("program")

#quectel add 
    elem_program = Element("program")
    elem_erase = Element("erase")
    ...
    for i in range(NumPartitions):
		...
		for pr_cmd in "erase", "program":
			...
			if user_partition_entries[i]["filename"] == "partition.mbn" and pr_cmd == "program": 
				elem.attrib["filename"]             = partition_filename
			elif pr_cmd == "program":
				if user_partition_entries[i]["filename"]:
					add_str = "..\\"
				else:
					continue
				elem.attrib["filename"]             = add_str + user_partition_entries[i]["filename"]
				#if elem.attrib["filename"] == "sbl1.mbn":
				if elem.attrib["filename"][-8:] == ('sbl1.mbn'):
					sbl1_program = elem
					continue
			elif pr_cmd == "erase" and user_partition_entries[i]["filename"] == "":
				continue
			#elif pr_cmd == "erase" :
			#	continue
			NewXML.append(elem)
...
#---------------------- Quectel's 4+2 flash (pagesize 4k, blocksize 256k) FLASH PARAM ----------------------#
block_size_in_kb=256     #quectel: 4+2 flash (pagesize 4k, blocksize 256k)
page_size_in_kb=4        #quectel: 4+2 flash (pagesize 4k, blocksize 256k)
total_page_count=131072  #quectel: 4+2 flash total page count is 512M/4K=131072
#-----------------------------------------------------------------------------#
