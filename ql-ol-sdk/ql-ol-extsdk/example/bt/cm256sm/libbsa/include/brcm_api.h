...
#ifndef BTM_BLE_APCF_STR_LEN_MAX
#define BTM_BLE_APCF_STR_LEN_MAX     29  /* Currently the max number of characters is 29 */
#endif
#ifndef BTM_BLE_APCF_MAX_FILTER
#define BTM_BLE_APCF_MAX_FILTER      15  /* Currently the max filter index is 15 (0 -15) */
#endif
...
enum
{
    BTM_BLE_APCF_COND_ADD,
    BTM_BLE_APCF_COND_DELETE,
    BTM_BLE_APCF_COND_CLEAR = 2
};
typedef UINT8 tBTM_BLE_APCF_COND_OP;

typedef UINT8 tBTM_BLE_APCF_FILT_INDEX;

/* filter selection bit index  */
enum
{
    BTM_BLE_APCF_ENABLE,
    BTM_BLE_APCF_FILTER_SETTING,
    BTM_BLE_APCF_ADDR_FILTER,
    BTM_BLE_APCF_SRVC_UUID,
    BTM_BLE_APCF_SRVC_SOL_UUID,
    BTM_BLE_APCF_LOCAL_NAME,
    BTM_BLE_APCF_MANU_DATA,
    BTM_BLE_APCF_SRVC_DATA
};

typedef UINT8   tBTM_BLE_APCF_COND_TYPE;

#define BTM_BLE_APCF_LOGIC_OR             0
#define BTM_BLE_APCF_LOGIC_AND            1
typedef UINT8 tBTM_BLE_APCF_LOGIC_TYPE;
...
#define BTM_BLE_APCF_CONFIG       2
typedef UINT8 tBTM_BLE_APCF_ACTION;

/* BLE APCF CFG operation complete callback */
typedef void (tBTM_BLE_APCF_CFG_CMPL_CBACK)(tBTM_BLE_APCF_COND_OP action,
                                                                                tBTM_BLE_APCF_COND_TYPE cond_type,
                                                                                tBTM_STATUS status, UINT8 num_avail);
/* BLE APCF ENABLE operation complete callback */
typedef void (tBTM_BLE_APCF_ENABLE_CMPL_CBACK)(BOOLEAN enable, tBTM_STATUS status);
...
typedef union
{
      ...
      UINT32              uuid32_mask;
      UINT8               uuid128_mask[LEN_UUID_128];
}tBTM_BLE_APCF_COND_MASK;

typedef struct
{
    tBT_UUID                uuid;           /* UUID condition */
    tBTM_BLE_APCF_COND_MASK   p_uuid_mask;           /* UUID mask */
}tBTM_BLE_APCF_UUID_COND;

typedef struct
{
    UINT8    data_len;       /* <= 29 bytes */
    UINT8    *p_data;
}tBTM_BLE_APCF_LOCAL_NAME_COND;

typedef struct
{
    UINT8    data_len;       /* <= 29 bytes */
    UINT8    *p_data;        /* manufactuer/service data */
    UINT8    *p_data_mask;  /* manufactuer/service data matching mask, same length as
                     manufactuer/service data pattern, set to all 0xff, match exact the same data */
}tBTM_BLE_APCF_DATA_COND;
...
#define BTM_BLE_APCF_BRDCAST_ADDR_FILT   1
#define BTM_BLE_APCF_SERV_DATA_CHG_FILT 2
#define BTM_BLE_APCF_SERV_UUID                    4
#define BTM_BLE_APCF_SERV_SOLC_UUID         8
#define BTM_BLE_APCF_LOC_NAME_CHECK        16
#define BTM_BLE_APCF_MANUF_NAME_CHECK  32
#define BTM_BLE_APCF_SERV_DATA_CHECK      64
typedef UINT16 tBTM_BLE_APCF_FEAT_SEL;

#define BTM_BLE_APCF_LIST_LOGIC_OR   0
#define BTM_BLE_APCF_LIST_LOGIC_AND  1
typedef UINT16 tBTM_BLE_APCF_LIST_LOGIC_TYPE;
#define BTM_BLE_APCF_FILT_LOGIC_OR   0
#define BTM_BLE_APCF_FILT_LOGIC_AND  1
typedef UINT8 tBTM_BLE_APCF_FILT_LOGIC_TYPE;
typedef UINT8  tBTM_BLE_APCF_RSSI_THRESHOLD;

typedef struct
{
    tBTM_BLE_APCF_FEAT_SEL                feat_seln;
    tBTM_BLE_APCF_LIST_LOGIC_TYPE         logic_type;
    tBTM_BLE_APCF_FILT_LOGIC_TYPE         filt_logic_type;
    tBTM_BLE_APCF_RSSI_THRESHOLD          rssi_high_thres;
}tBTM_BLE_APCF_FILT_PARAMS;

typedef union
{
    tBLE_BD_ADDR                        target_addr;
    tBTM_BLE_APCF_LOCAL_NAME_COND       local_name; /* lcoal name filtering */
    tBTM_BLE_APCF_DATA_COND             data;       /* manufactuer/service data filtering */
    tBTM_BLE_APCF_UUID_COND             uuid;       /* service/solicitated UUID filtering */
    tBTM_BLE_APCF_FILT_PARAMS           filter_setting;
...
/*******************************************************************************
**
** Function         BTM_BleAPCFEnable
**
** Description     This function is called to enable APCF
**
...
**
*******************************************************************************/
BTM_API extern tBTM_STATUS BTM_BleAPCFEnable(BOOLEAN enable,
                           tBTM_BLE_APCF_ENABLE_CMPL_CBACK *p_cmpl_cback);

/*******************************************************************************
**
** Function         BTM_BleAPCFCfgFilter
**
** Description      This function is called to configure the APCF filter condition.
**
** Parameters      action: to read/write/clear
**                      cond_type: filter condition type.
**                      filter_index : index of filter
**                      p_cond: filter condition paramter
**
** Returns          void
**
*******************************************************************************/
tBTM_STATUS BTM_BleAPCFCfgFilter(tBTM_BLE_APCF_COND_OP action,
                                      tBTM_BLE_APCF_COND_TYPE cond_type,
                                      tBTM_BLE_APCF_FILT_INDEX filter_index,
                                      tBTM_BLE_APCF_COND_PARAM *p_cond,
                                      tBTM_BLE_APCF_CFG_CMPL_CBACK *p_cmpl_cback);
