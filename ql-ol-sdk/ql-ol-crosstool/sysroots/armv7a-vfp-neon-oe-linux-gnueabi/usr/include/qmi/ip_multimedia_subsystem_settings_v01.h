...
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //source/qcom/qct/interfaces/qmi/imss/main/latest/api/ip_multimedia_subsystem_settings_v01.h#53 $
 *====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
...
#define IMSS_V01_IDL_MINOR_VERS 0x26
/** Major Version Number of the qmi_idl_compiler used to generate this file */
#define IMSS_V01_IDL_TOOL_VERS 0x06
/** Maximum Defined Message ID */
#define IMSS_V01_MAX_MESSAGE_ID 0x0062
/**
    @}
  */
...
typedef enum {
  ...
  IMS_SETTINGS_HO_ENABLED_ALL_V01 = 1, /**<  HO enabled from WWAN to WLAN and WLAN to WWAN  */
  IMS_SETTINGS_HO_DISABLED_ALL_V01 = 2, /**<  HO disabled from WWAN to WLAN and WLAN to WWAN   */
  IMS_SETTINGS_HO_ENABLED_WLAN_TO_WWAN_ONLY_V01 = 3, /**<  HO enabled from WLAN to WWAN ONLY */
  IMS_SETTINGS_HO_ENABLED_WWAN_TO_WLAN_ONLY_V01 = 4, /**<  HO enabled from WWAN to WLAN ONLY */
 ...
 */
  ...
  /**<   Standalone message VoWifi ProvId.
    */
...
typedef struct {
  ...
  /**<
        \hl{Number of currently available H.264 configurations. This TLV is used to
...
typedef struct {
  ...
  /**<   Hysteresis timer for WWAN to WLAN handover, in seconds.
	     This timer is started if LTE to WLAN idle mode handover
	     happens due to LTE RAT loss. When in cellular-preferred mode, WLAN to LTE
		 handover will only happen after LTE has been available for this period of time.
		 If this value is not present, a default of 120 seconds is used.
	*/
  ...
  /**<   Hysteresis timer for moving voice services from WLAN to 1x, in seconds.
	     This timer is started if 1x system is used for voice service and
		 1x quality drops below a threshold in idle mode. When in cellular-preferred mode,
		 voice service will move to 1x only after 1x has been available with improved quality for this period of time.
		 If this value is not present, a default of 120 seconds is used.
	*/
  ...
  /**<   IMS Handover hysteresis timer WWAN to WLAN, in seconds.
	     In Wi-Fi preferred mode, this timer is started if WLAN to LTE idle mode handover
	     happens due to WLAN RAT loss. LTE to WLAN handover will only happen after WLAN
		 has been available for this period of time.
		 If this value is not present, a default of 120 seconds is used.
	*/
  ...
  /**<   IMS Handover mode. Values: \n
      - IMS_SETTINGS_HO_ENABLED_ALL (1) --  HO enabled from WWAN to WLAN and WLAN to WWAN
      - IMS_SETTINGS_HO_DISABLED_ALL (2) --  HO disabled from WWAN to WLAN and WLAN to WWAN
      - IMS_SETTINGS_HO_ENABLED_WLAN_TO_WWAN_ONLY (3) --  HO enabled from WLAN to WWAN ONLY
      - IMS_SETTINGS_HO_ENABLED_WWAN_TO_WLAN_ONLY (4) --  HO enabled from WWAN to WLAN ONLY
 */
...
/** @addtogroup imss_qmi_messages
    @{
  */
/** Request Message; Sets the IMS Application's VoLTE for Internet Connected Endpoint configuration parameters for the requesting
             control point. */
typedef struct {

  /* Optional */
  /*  IMS Application's VoLTE for Internet Connected Endpoint enabled or not */
  ...
  /**<   Indicates whether the vice is enabled. */
...
/** @addtogroup imss_qmi_messages
    @{
  */
/** Response Message; Sets the IMS Application's VoLTE for Internet Connected Endpoint configuration parameters for the requesting
             control point. */
typedef struct {
...
/** @addtogroup imss_qmi_messages
    @{
  */
/** Request Message; Retrieves the IMS Application's VoLTE for Internet Connected Endpoint configuration parameters. */
typedef struct {
  /* This element is a placeholder to prevent the declaration of
     an empty struct.  DO NOT USE THIS FIELD UNDER ANY CIRCUMSTANCE */
  char __placeholder;
...
/** @addtogroup imss_qmi_messages
    @{
  */
/** Response Message; Retrieves the IMS Application's VoLTE for Internet Connected Endpoint configuration parameters. */
typedef struct {
...
typedef struct {
  ...
  */
  ...
  /* Optional */
  /*  VoLTE for Internet Connected Endpoint Configuration */
...
/** @addtogroup imss_qmi_messages
    @{
  */
/** Indication Message; Indicates when the IMS Application's VoLTE for Internet Connected Endpoint configuration parameters change. */
typedef struct {

  /* Optional */
  /*  IMS IMS Application's VoLTE for Internet Connected Endpoint enabled or not */
  uint8_t qipcall_vice_enabled_valid;  /**< Must be set to true if qipcall_vice_enabled is being passed */
  uint8_t qipcall_vice_enabled;
  /**<   Indicates whether the vice is enabled. */
