...
//add by len for manual recovery mode
#define QUECTEL_SYSTEM_UPGRADE_MODE     0
#define QUECTEL_SYSTEM_RECOVERY_MODE    1
#define QUECTEL_SYSTEM_NORMAL_MODE      2

unsigned int system_mode_flag = QUECTEL_SYSTEM_NORMAL_MODE;
//add end
...
static int __init quectel_get_rootfstype(char *str)
{
	strcpy(rootfstype, str);
	rootfstype[strlen(str)] = '\0';
    return 0;
}
early_param("rootfstype", quectel_get_rootfstype);

static int __init quectel_get_rootflags(char *str)
{
	strcpy(rootflags, str);
	rootflags[strlen(str)]='\0';
    return 0;
}
early_param("rootflags", quectel_get_rootflags);

#ifndef QUECTEL_FLASH_SUPPORT_1G
//add for manual recovery mode
static int __init quectel_get_recovery(char *str)
{
    if (str)
    {
        unsigned int val;
        kstrtouint(str,10,&val);
        system_mode_flag = val;
    }
    return 0;
}
early_param("recovery", quectel_get_recovery);
#endif
//add end
...
static int __ref kernel_init(void *unused)
{
    ...
    //if(strstr(temp_cmdLine, "ubi.mtd=19")) //this condition will replace last,
    if ( system_mode_flag == QUECTEL_SYSTEM_NORMAL_MODE )
    {
      /* µķsystemΪ18 system132+1 flashmodemʾģеģʽrecovery ģʽmodem
        ...
        */
        ...
        //modify by len[QUECTEL], 2018-03-06
	...
	{
		printk("@Ramos set restore recoveryfs flag here 444444 \r\n");
		Quectel_Set_Partition_RestoreFlag("recoveryfs",4);
	}
#endif
