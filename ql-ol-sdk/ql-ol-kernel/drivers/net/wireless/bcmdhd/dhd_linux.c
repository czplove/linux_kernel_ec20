...
#endif
#ifdef USE_CID_CHECKdhd_module_exit
 ...
 */
#if defined(CUSTOMER_HW4) || defined(CUSTOMER_HW5) || defined(CUSTOMER_IPCAM)
				int resched_cnt = 0;
...
static int dhd_preinit_proc(dhd_pub_t *dhd, int ifidx, char *name, char *value)
{
	...
	if (!strcmp(name, "country")) {
		...
		if ((ret  = dhd_iovar(dhd, 0, "btamp_chan", (char *)&btamp_chan, sizeof(btamp_chan),
				NULL, 0, TRUE) < 0))
...
/* Set frameburst to value */
	...
	if (dhd_use_idsup == 1) {
		ret = dhd_iovar(dhd, 0, "sup_wpa", NULL, 0,
				(char *)&iovbuf, sizeof(iovbuf), FALSE);
...
#ifdef SUPPORT_2G_VHT
	if ((ret =  dhd_iovar(dhd, 0, "vht_features", (char *)&vht_features,
		sizeof(vht_features), NULL, 0, TRUE)) < 0) {
...
#if defined(CUSTOMER_HW4) && defined(ENABLE_BCN_LI_BCN_WAKEUP)
	dhd_iovar(dhd, 0, "bcn_li_bcn", (char *)&bcn_li_bcn, sizeof(bcn_li_bcn),
		NULL, 0, TRUE);
#endif /* CUSTOMER_HW4 && ENABLE_BCN_LI_BCN_WAKEUP */
#ifdef AMPDU_VO_ENABLE
	tid.tid = PRIO_8021D_VO; /* Enable TID(6) for voice */
	tid.enable = TRUE;
	dhd_iovar(dhd, 0, "ampdu_tid", (char *)&tid, sizeof(tid),
		NULL, 0, TRUE);
	...
	if (dhd->op_mode & DHD_FLAG_HOSTAP_MODE) {
		...
		memset(buf, 0, sizeof(buf));
		ret = dhd_iovar(dhd, 0, "bus:txglom_auto_control", NULL, 0,
			buf, sizeof(buf), FALSE);
		if (ret < 0) {
			glom = 0;
			dhd_iovar(dhd, 0, "bus:txglom", (char *)&glom, sizeof(glom),
				NULL, 0, TRUE);
		} else {
			if (buf[0] == 0) {
				glom = 1;
	...
	switch (flag) {
		...
		case 0: /* Deepsleep Off */
			...
			for (cnt = 0; cnt < MAX_TRY_CNT; cnt++) {
				...
				ret = dhd_iovar(dhdp, 0, "deepsleep", NULL, 0,
...
dhd_force_disable_singlcore_scan(dhd_pub_t *dhd)
{
	...
	if (IS_ERR(fp)) {
		...
		if (ret > 0 && NULL != strstr(vender, "wisol")) {
			...
			ret = dhd_iovar(dhd, 0, "scan_ps", (char *)&scan_ps, sizeof(scan_ps),
				NULL, 0, TRUE);
			if (ret < 0)
